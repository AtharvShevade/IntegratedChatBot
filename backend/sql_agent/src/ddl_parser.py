"""
Vendored copy of the dev repo's embedding_building/parser.py, frozen into this
runtime package because sql_generator.py's DDL column-type fallback is the one
live runtime import this package has out of build-time tooling. This module
must stay self-contained (no other embedding_building imports) — it is not
re-synced automatically if the build-time original changes; re-vendor by hand
if parser.py's parsing logic is fixed or extended upstream.
"""
import re
