"""Phase 1 data-layer tests — run against the REAL 6.0 data trees on this
machine, not fixtures. Skips (not fails) when a tree isn't present, so this
suite is safe to run on a machine without the real repos mounted.
...
def test_attribute_map_excludes_credentials():
    ...
    for entity_name, spec in v6_0_schema.SCHEMA.items():
        raw_values = {v for v in spec.attribute_map.values() if v is not None}
        leaked = raw_values & CREDENTIAL_FIELDS
        assert not leaked, (
            f"v6_0.{entity_name}.attribute_map maps a credential field: {leaked}"
        )
