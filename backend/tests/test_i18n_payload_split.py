"""Regression tests for the prose/options translation split (PRODUCTION).

Ported verbatim from eval/multilingual/tests/test_payload_split.py, retargeted
at backend.i18n. The production payload.py is a byte-for-byte copy of the
evaluated one (asserted below), so the behaviour measured on the 24-case
FR/AR/HI run is the behaviour shipping here.
...
from backend.i18n import boundary, payload
...
def test_translatable_payload_still_selects_the_same_fields():
    ...
    got = boundary.translatable_payload(response)
...
# --------------------------------------------------------------------------
# The production module IS the evaluated module
# --------------------------------------------------------------------------

def test_production_payload_is_byte_identical_to_the_evaluated_one():
    """payload.py was copied into backend/i18n/ unmodified. If the two ever
    diverge, the 24-case measurements no longer describe what ships."""
    import pathlib
    prod = pathlib.Path(payload.__file__)
    evaluated = prod.parents[2] / "eval" / "multilingual" / "payload.py"
    if not evaluated.exists():
        pytest.skip("evaluation harness not present in this checkout")
    assert prod.read_bytes() == evaluated.read_bytes()
